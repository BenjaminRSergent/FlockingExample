...
public abstract class Locomotion {
    ...
    private final static float MIN_VELO_SQ = 1;

    public abstract void update(Boid boid, World world, float deltaTime);

    private static Vector2 motion = new Vector2();

    public static void integrate(Boid boid,  float deltaTime) {
        Vector2 acceleration = boid.acceleration;
        ...
        if (boid.acceleration.len2() < 1 && boid.velocity.len2() < MIN_VELO_SQ) {
